...
<script>
    ...
    app.controller("angController", function ($scope, $http, $filter, $window) {
        ...
        $scope.getGroupedPackages = function () {
            function setPackages(res) {
                $scope.distrPackages = res.data.distrPackages;
                $scope.oneTimepackages = res.data.oneTimepackages;
            }

            ajaxCall($http, "package/get-grouped-packages", null, setPackages);
        };
...
//      $scope.getPackages();
        ...
        $scope.changeStatusHistoryDate = function () {
            if ($scope.slcted != undefined) {

                if ($scope.prevDate != undefined) {
                    var enteredDate = new Date($scope.changeStatusHistDate.split(/\//).reverse().join('-'));
                    var prevDate = new Date($scope.prevDate);
                    if (enteredDate != undefined && prevDate != undefined && enteredDate < prevDate) {
                        errorMsg('არასწორი თარიღი, გადაფარვა წინა ცვლილების თარიღთან ');
                        return;
                    }
                }
                ...
                function resFnc(res) {
                    if (res.errorCode == 0) {
                        successMsg('Operation Successfull');
                        closeModal('editStatusHistoryModal');
                        $scope.loadMainData();
                    } else {
                        errorMsg(res.message);
                    }
                ...
                if ($scope.changeStatusHistDate != undefined && $scope.changeStatusHistDate.includes('/')) {
                    $scope.changeStatusHistDate = $scope.changeStatusHistDate.split(/\//).reverse().join('-');
                }
                if ($scope.changeStatusHistDate == undefined || $scope.changeStatusHistDate.length < 1) {
                    errorMsg('თარიღის მითითება სავალდებულოა');
                    return;
                }

                ajaxCall($http, "abonent/change-status-history?id=" + $scope.statHistoryId + "&date=" + $scope.changeStatusHistDate, null, resFnc);
            }
        }

        $scope.showLastDateToChange = function (hist, prevObj) {
            $scope.prevDate = new Date(prevObj.disableDate.split(/\//).reverse().join('-'));
            $scope.changeStatusHistDate = hist.disableDate;
            $scope.statHistoryId = hist.id;
            closeModal("detailModal");
        }

        $scope.daricxva = function (id) {
            if (confirm("ადასტურებთ დარიცხვას ?")) {
                if (id != undefined && $scope.request.id != undefined) {
                    function resFnc(res) {
                        if (res.errorCode == 0) {
                            successMsg('Operation Successfull');
                            closeModal("packages");
                    ...
                    ajaxCall($http, "abonent/daricxva?abonentId=" + $scope.request.id + "&packageId=" + id, null, resFnc);
                } else {
                    errorMsg('პაკეტის იდენტიფიცირება ვერ ხერხდება !!!')
                }
            }
        };
                ...
                <div class="row">
                    ...
                    <div class="col-md-12" id="historyDivId">
                        ...
                        <table ng-repeat="v in uniqAbonentsHistory"
                            ...
                            <tbody>
                            <tr ng-repeat="r in historylist | filter:{abonent:{id:v}}:true " ng-if="$index == 0"
                                class="text-center">
                                <td></td>
                                <td>{{r.abonent.id}}</td>
                        ...
                        <div class="form-group col-sm-12 ">
                            ...
                            <div class="col-sm-11 list-group">
                                <table class="table table-bordered table-hover">
                                    <tr ng-repeat="t in oneTimepackages">
                                        <td>{{t.name}}&nbsp;(იურ:{{t.juridicalPrice}}ლ. /
                                            ფიზ:{{t.personalPrice}}ლ.)
                                        </td>
                                        <td class="col-md-1">
                                            <button type="button" class="btn btn-block btn-default btn-md"
                                                    ng-click="daricxva(t.id)">
                            ...
                            <label class="control-label col-sm-1"></label>
                            <div class="col-sm-11 list-group">
                                <label ng-repeat="t in distrPackages" class="col-sm-12 list-group-item">
                            ...
                            <td>
                                <ul>
                                    <li ng-repeat="k in slcted.statusHistory">
                                        {{k.status.name}} - {{k.disableDate}} - (By User: {{k.user.userDesc}}) &nbsp;&nbsp;
                                        <a data-toggle="modal" title="რედაქტირება" ng-if="$last"
                                           ng-click="showLastDateToChange(k, slcted.statusHistory[$index - 1])"
                                           data-target="#editStatusHistoryModal" class="btn btn-xs">
                            ...
                            <td>
                                <ul>
                                    <li ng-repeat="t in slcted.abonentPackages">
                                        {{t.type.name}} - {{t.name}}&nbsp;(იურ:{{t.juridicalPrice}}ლ. /
                                        ფიზ:{{t.personalPrice}}ლ.)
...
<div class="modal fade bs-example-modal-lg not-printable" id="editStatusHistoryModal" role="dialog"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="editStatusHistoryModalLabel">შეავსეთ ინფორმაცია</h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <form class="form-horizontal" name="editFormName">
                        <div class="form-group col-sm-10 ">
                            <label class="control-label col-sm-3">ოპერაციის თარიღი</label>
                            <div class="col-sm-9">
                                <input type="text" ng-model="changeStatusHistDate"
                                       class="form-control input-sm dateInput"/>
                            </div>
                        </div>
                        ...
                        <div class="form-group col-sm-10"></div>
                        <div class="form-group col-sm-12 text-center">
                            <a class="btn btn-app" ng-click="changeStatusHistoryDate()">
                        ...
                        <div class="form-group col-sm-10 ">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" ng-model="request.streetId">
                                    <option ng-repeat="s in streets" ng-selected="s.id === request.streetId"
                                            ng-value="s.id">
                                        {{s.name}}
                                    </option>
                                </select>
                        ...
                        <div class="form-group col-sm-10 ">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" ng-model="request.districtId">
                                    <option ng-repeat="s in districts" ng-selected="s.id === request.districtId"
                                            ng-value="s.id">
                    ...
                    <div id="filter-panel" class="filter-panel">
                        <div class="panel panel-default">
                            <div class="panel-body">
                                ...
                                <%--</div>--%>
                                <div class="form-group col-md-2">
                                    <select class="form-control" ng-model="srchCase.hasBill"
                                            ng-change="srchBtnClicked()">
                                        <option value="" selected="selected">დავალ / ავანსი</option>
                                        <option ng-selected="1 === srchCase.hasBill" value="{{1}}">დავალიანება</option>
                                        <option ng-selected="2 === srchCase.hasBill" value="{{2}}">ავანსი</option>
                                    </select>
                                </div>
                                <div class="form-group col-md-2">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tr ng-repeat="r in list" ng-dblclick="handleDoubleClick(r.id)">
                            ...
                            <td class="text-center">
                                ...
                                <%--<c:if test="<%= isAdmin %>">--%>
                                <a ng-click="edit(r.id)" data-toggle="modal" title="რედაქტირება"
