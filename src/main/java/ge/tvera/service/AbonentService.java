...
//      ******************************************************************************************************************
  ...
  public MonthlyBills updateMonthlyBill(MonthlyBillsDTO request, Integer userId) {
    ...
    if (monthlyBills != null) {
      ...
      monthlyBills.setOperDate(request.getOperDate());
...
//  }
  ...
  public void daricxva(Integer abonentId, Integer packageId, Integer userId) {
    ...
    if (obj != null && pack != null) {
      ...
      //  insert row into monthly bills for this oneTime bill
      Calendar cal = Calendar.getInstance();
      cal.set(Calendar.DAY_OF_MONTH, 15);

      MonthlyBills bills = new MonthlyBills();
      bills.setActionUser(userId);
      bills.setAmount(increment);
      bills.setAbonent(obj);
      bills.setOperDate(new java.sql.Date(cal.getTimeInMillis()));
      bills.setPurpose(pack.getGroup().getName());
      abonentDAO.create(bills);
  ...
  public void changeServiceStatus(int id, Date disableDate, int userId) throws IncorrectDateException {
    ...
    prevMonthLastDay.set(Calendar.MONTH, -1);
      ...
      if (!isToday) {
        if (oldDate) {
          ...
          if (obj.getBill() != null) {
            Double dailyBill = obj.getBill() / daysCountInMonth;
            obj.setBalance(obj.getBalance() - (dailyBill * daysCountBetween));
            int daysAfteFirstDayInCrntMonth = 1;
            if (newDisDateCal.after(prevMonthLastDay)) {
              daysAfteFirstDayInCrntMonth = daysBetween(getZeroTimeDate(newDisDateCal.getTime()), getZeroTimeDate(new java.util.Date()));
            } else {
          ...
          if (obj.getBill() != null) {
            ...
            obj.setCollectedBill((obj.getCollectedBill() + (dailyBill * (daysCountBetween + 1))) > obj.getBill() ?
                    obj.getBill() : (obj.getCollectedBill() + (dailyBill * (daysCountBetween + 1))));
          }
        }
      }
        ...
        if (!isToday) {
          if (oldDate) {
            ...
            if (obj.getBill() != null) {
              ...
              obj.setCollectedBill((obj.getCollectedBill() + (dailyBill * daysAfteFirstDayInCrntMonth)) > obj.getBill() ?
                      obj.getBill() : (obj.getCollectedBill() + (dailyBill * daysAfteFirstDayInCrntMonth)));
            }
          } else {
